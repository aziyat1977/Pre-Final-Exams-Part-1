...
import { GraduationCap, Sword, Ghost, Blocks, ChevronRight, Sparkles, Volume2, Brain, Lock, Zap, BookOpen } from 'lucide-react';
...
import { QUANTITY_DATA, EXAM_KILLERS, DETAILED_TENSES, DETAILED_CONNECTORS, QUIZ_QUESTIONS, INTERMEDIATE_LESSONS, PRE_INTERMEDIATE_LESSONS } from './constants';
...
const App: React.FC = () => {
  ...
  const getBackgroundClass = () => {
    switch (view) {
      ...
      case 'pre-intermediate': return 'bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-teal-900 via-gray-900 to-black';
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'pre-intermediate':
        if (selectedTenseId) {
            const lesson = PRE_INTERMEDIATE_LESSONS.find(l => l.id === selectedTenseId);
            if (lesson) {
        ...
        return (
          <div className="space-y-8 animate-fade-in-up">
            <div className="text-center mb-12">
              <h1 className="text-4xl md:text-7xl font-hud text-white mb-4 text-shadow-lg tracking-wider">
                <span className="text-teal-400">NAVIGATE A2</span> COURSE
              </h1>
              <p className="text-gray-400 font-mono text-xl">Pre-Intermediate Syllabus // Oxford University Press</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
               {PRE_INTERMEDIATE_LESSONS.map((lesson, i) => (
                 <motion.div 
                    key={lesson.id}
                    ...
                    className="bg-gray-800/80 border-2 border-teal-600/50 p-6 rounded-xl relative overflow-hidden group hover:border-teal-400 transition-all cursor-pointer hover:bg-teal-900/10 h-full flex flex-col"
                 >
                    <div className="flex justify-between items-start mb-4">
                        <span className="bg-teal-500 text-black text-xs px-2 py-1 rounded font-bold uppercase tracking-widest">{lesson.id}</span>
                        <BookOpen size={20} className="text-teal-500/50" />
                    </div>
                    <h3 className="text-xl font-hud text-white mb-2 leading-tight">{lesson.title}</h3>
                    <p className="text-gray-400 font-mono text-xs flex-1">{lesson.topicDescription}</p>
                    <div className="mt-4 pt-4 border-t border-white/10 flex justify-between items-center text-xs text-teal-400 font-bold">
               ...
               <motion.div 
                  ...
                  className="bg-teal-900/40 border-2 border-teal-500 p-6 rounded-xl relative overflow-hidden cursor-pointer hover:bg-teal-900/60 hover:border-teal-400 transition-all group col-span-1 md:col-span-2 lg:col-span-3 flex items-center justify-between"
               >
                   <div>
                        <h3 className="text-2xl font-hud text-white flex items-center gap-2"><Zap className="text-teal-400" /> TEST GENERATOR</h3>
                        <p className="text-gray-400 font-mono text-sm">Create infinite custom exams based on A2 syllabus.</p>
                   </div>
      ...
      case 'intermediate':
        ...
        return (
          <div className="space-y-8 animate-fade-in-up">
            <div className="text-center mb-12">
              <h1 className="text-4xl md:text-7xl font-hud text-white mb-4 text-shadow-lg tracking-wider">
                <span className="text-yellow-500">NAVIGATE B1</span> COURSE
              </h1>
              <p className="text-gray-400 font-mono text-xl">Official Syllabus // Oxford University Press</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
               {INTERMEDIATE_LESSONS.map((lesson, i) => (
                 <motion.div 
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ delay: i * 0.05 }}
                    className="bg-gray-800/80 border-2 border-yellow-600/50 p-6 rounded-xl relative overflow-hidden group hover:border-yellow-500 transition-all cursor-pointer hover:bg-yellow-900/10 h-full flex flex-col"
                 >
                    <div className="flex justify-between items-start mb-4">
                        <span className="bg-yellow-500 text-black text-xs px-2 py-1 rounded font-bold uppercase tracking-widest">{lesson.id}</span>
                        <BookOpen size={20} className="text-yellow-500/50" />
                    </div>
                    <h3 className="text-xl font-hud text-white mb-2 leading-tight">{lesson.title}</h3>
                    <p className="text-gray-400 font-mono text-xs flex-1">{lesson.topicDescription}</p>
                    <div className="mt-4 pt-4 border-t border-white/10 flex justify-between items-center text-xs text-yellow-500 font-bold">
                        <span>START UNIT</span>
                        <ChevronRight size={16} />
                    </div>
                 </motion.div>
               ...
               <motion.div 
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ delay: 0.4 }}
                  className="bg-blue-900/40 border-2 border-blue-500 p-6 rounded-xl relative overflow-hidden cursor-pointer hover:bg-blue-900/60 hover:border-blue-400 transition-all group col-span-1 md:col-span-2 lg:col-span-3 flex items-center justify-between"
               >
                   <div>
                        <h3 className="text-2xl font-hud text-white flex items-center gap-2"><Zap className="text-blue-400" /> TEST GENERATOR</h3>
                        <p className="text-gray-400 font-mono text-sm">Create infinite custom exams based on B1/B1+ syllabus.</p>
                   </div>
          ...
          <div className="max-w-6xl mx-auto pt-12 text-center space-y-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-4">
               ...
               <div className="grid gap-4">
                  <Button 
                    label="ENTER NAVIGATE PRE-INT" 
                    variant="standoff" 
                    icon={<Brain />} 
                    onClick={() => setView('pre-intermediate')} 
                    className="h-full text-sm"
                  />
                  <Button 
                    label="ENTER NAVIGATE B1 COURSE" 
                    variant="primary" 
                    icon={<Brain />} 
